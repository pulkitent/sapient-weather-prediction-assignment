...
@Component
public class MaxTemperatureConditionEvaluator implements IWeatherConditionEvaluator {

  private static final Logger LOGGER = LoggerFactory.getLogger(MaxTemperatureConditionEvaluator.class);
  ...
  public List<String> getAdvices(WeatherPredictionDataDTO weatherPredictionDTO, Condition condition) {
    ...
    for (Rule rule : condition.getRules()) {
      switch (rule.getOperator()) {
        case Constant.EQUAL:
          if (Double.compare(weatherPredictionDTO.getMaxTemperature(), Double.parseDouble(rule.getValue())) == 0) {
